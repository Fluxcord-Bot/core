...
\`\`\``);
      ...
      channel.send({
        content: `${isFluxer ? "Fluxer" : "Discord"} channel ${message.channel instanceof FluxerChannel ?
          message.channel.name : message.channel instanceof DiscordGuildChannel ? message.channel.name : ""
...
To approve the bridge, run \`${Config.BotPrefix}verify\`! If not, just ignore this message and it will be cancelled after 2 minutes.`,
    ...
    case "verify": {
      ...
      if (!bridgeMap) {
        await message.reply('This server isn\'t configured for bridging. Bridge this first before verifying.')
        break;
      }

      const channelId = isFluxer ? bridgeMap.discordChannel : bridgeMap.fluxerChannel
      ...
      if (!thisChannel || !channel || !channel.isSendable()) {
        log(isFluxer ? 'FLUXER' : 'DISCORD', `channel ${message.channelId} is on non expected value. report it on https://codeberg.org/jbcarreon123/fluxcord as this SHOULD NOT happen`)
        return;
      }
      ...
      if (thisChannel instanceof FluxerGuildChannel && type !== 'FLUXER2DISCORD') {
      ...
      } else if (thisChannel instanceof DiscordGuildChannel && thisChannel.isTextBased() && type !== 'DISCORD2FLUXER') {
        const webhook = await thisChannel.createWebhook({
      ...
      } else if (channel instanceof DiscordGuildChannel && channel.isTextBased() && type !== 'DISCORD2FLUXER') {
        const webhook = await channel.createWebhook({
      ...
      await channel.send({
        content: '🎉 This channel is now bridged to ' + (isFluxer ? 'Fluxer': 'Discord') + '!'
      })
